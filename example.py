...
def test3(dataset, alg, seed, nEvals=50000, store=False):
    ...
    mpaes_rcs_fdt.score(Xte, yte)

def test4(dataset, alg, seed, nEvals=50000, store=False):
    X_Tra, y_Tra, attributes, inputs, outputs = load_dataset('GeneExpre_Tra')  # iris
    X_Tst, y_Tst, attributes, inputs, outputs = load_dataset('GeneExpre_Tst')  # iris
    ...
    # mpaes_rcs_fdt.show_pareto_archives(Xte, yte)
    # mpaes_rcs_fdt.show_model('median', inputs, outputs)
    score_test = mpaes_rcs_fdt.score(Xte, yte)
    score_train = mpaes_rcs_fdt.score(Xtr, ytr)
...
if __name__=="__main__":
    ...
    score_train,score_test=test4('iris', 'mpaes22', 2, nEvals=1000, store=False)
    print(max(score_train, key=lambda x:x[0]))  #0 accuracy  1 auc   2 trl
    print(max(score_train, key=lambda x:x[1]) ) #0 accuracy  1 auc   2 trl
    print(min(score_train, key=lambda x:x[2]))  #0 accuracy  1 auc   2 trl